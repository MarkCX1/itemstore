...
net.Receive( "ItemStoreOpen", function()
	...
	if not hideinv then
		...
		function inv:Think()
			...
			local totalHeight = panel:GetTall() + 10 + inv:GetTall()
			local startY = math.max( 5, ( ScrH() / 2 ) - ( totalHeight / 2 ) )

			panel:SetPos( ScrW() / 2 - panel:GetWide() / 2, startY )
			inv:SetPos( ScrW() / 2 - inv:GetWide() / 2, startY + panel:GetTall() + 10 )
		end
